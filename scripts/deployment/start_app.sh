...
if [ -f .env ]; then
    ...
    print_warning ".env file not found. Using config/config.yaml and defaults."
fi
...
if [ -z "$DATABASE_URL" ]; then
    print_warning "DATABASE_URL not set, using production PostgreSQL connection string."
...
if command -v brew &> /dev/null; then
    ...
    if brew services list | grep -q 'postgresql@15.*started'; then
        print_success "PostgreSQL@15 service is running"
    elif brew services list | grep -q 'postgresql.*started'; then
        print_success "PostgreSQL service is running"
    else
        print_warning "PostgreSQL service is not running"
        print_status "Starting PostgreSQL@15 service..."
        brew services start postgresql@15 2>/dev/null || brew services start postgresql 2>/dev/null
        sleep 3
        if brew services list | grep -q 'postgresql@15.*started'; then
            print_success "PostgreSQL@15 service started successfully"
        elif brew services list | grep -q 'postgresql.*started'; then
            print_success "PostgreSQL service started successfully"
        else
            print_error "Failed to start PostgreSQL service"
            print_status "Please start PostgreSQL manually: brew services start postgresql@15"
            exit 1
        fi
    fi
...
if python3 -c "import psycopg2, os; conn = psycopg2.connect(os.getenv('DATABASE_URL', 'postgresql://ahuther@localhost:5432/se_letters')); print('✅ PostgreSQL connection successful'); conn.close()" 2>/dev/null; then
