#!/usr/bin/env python3
"""
SE Letters Pipeline - Industrial Grade V2
Enhanced with show all products, Grok metadata, overview, and replacement suggestions
"""
...
class AIMetadata:
    ...
    grok_raw_data: str = ""  # Raw Grok response
...
class ProcessingResult:
    ...
    replacement_products: List[Dict[str, Any]] = None  # Commercialized alternatives
    product_count: int = 0
...
class DocumentThumbnailGenerator:
    ...
    def _generate_default_thumbnail(self, file_path: Path) -> str:
        ...
        svg = f'''<svg width="200" height="280" xmlns="http://www.w3.org/2000/svg">
            <rect width="200" height="280" fill="#2c3e50"/>
            <rect x="50" y="50" width="100" height="130" fill="none" stroke="#ecf0f1" stroke-width="3"/>
            <text x="100" y="200" text-anchor="middle" fill="#ecf0f1" font-size="16">{ext}</text>
        </svg>'''
...
class EnhancedRangeExtractor:
    """Industrial-grade range extraction with Grok simulation"""
    ...
    def __init__(self):
        self.range_patterns = [
            ...
            r'\b(SYMMETRA[\w\-\s]*)\b', r'\b(SILCON[\w\-\s]*)\b', r'\b(UPS[\w\-\s]*)\b'
        ]
    ...
    def extract_ranges_with_metadata(self, content: str, context: DocumentContext) -> Dict[str, Any]:
        """Extract ranges with complete AI metadata tracking including Grok simulation"""
        ...
        # Simulate Grok API response
        grok_response = self._simulate_grok_response(content, context)
        ai_metadata.grok_raw_data = grok_response
        ...
        ai_metadata.validation_flags = {
            ...
            'grok_api_simulated': True,
            'obsolescence_detected': any(term in content.lower() for term in ['obsolet', 'discontinu', 'end of life'])
        }
    ...
    def _simulate_grok_response(self, content: str, context: DocumentContext) -> str:
        """Simulate Grok API response for demonstration"""
        timestamp = datetime.now().isoformat()
        
        # Analyze content for obsolescence indicators
        obsolescence_terms = ['obsolet', 'discontinu', 'end of life', 'phase out', 'withdrawal']
        found_terms = [term for term in obsolescence_terms if term in content.lower()]
        
        # Extract potential ranges
        ranges_found = []
        for pattern in self.range_patterns:
            matches = re.findall(pattern, content.upper(), re.IGNORECASE)
            ranges_found.extend([match for match in matches if match])
        ...
        grok_response = {
            ...
            "usage": {
                ...
                "total_tokens": len(content.split()) + 150
            },
            "choices": [{
                "message": {
                    "role": "assistant",
                    "content": {
                        ...
                        "product_identification": {
                            "ranges_detected": list(set(ranges_found)),
                            "obsolescence_indicators": found_terms,
                        ...
                        "extraction_metadata": {
                            ...
                            "context_hints": [context.pl_services_hint] if context.pl_services_hint else []
                        },
                        "recommendations": {
                            "replacement_needed": len(found_terms) > 0,
                            "search_commercialized": True,
                            "business_impact": "high" if len(ranges_found) > 0 else "medium"
                        }
                    }
                }
            }]
        ...
        return json.dumps(grok_response, indent=2)
    ...
    def _filename_intelligent_fallback(self, filename: str) -> List[str]:
        ...
        filename_patterns = [
            ...
            (r'symmetra', 'SYMMETRA'), (r'compact', 'COMPACT')
        ]
    ...
    def _clean_range_name(self, range_str: str) -> str:
        ...
        normalizations = {
            ...
            'SYMMETRA': 'SYMMETRA', 'COMPACT': 'COMPACT'
        }
...
class DocumentProcessor:
    ...
    def _extract_text_robust(self, file_path: Path) -> str:
        ...
        if file_ext == '.docx':
            ...
            methods = [self._extract_doc_libreoffice]
...
class DuckDBService:
    """Industrial-grade DuckDB service with replacement product suggestions"""
    ...
    def find_products_with_context(self, ranges: List[str], context: DocumentContext) -> List[Dict[str, Any]]:
        ...
        for range_name in ranges:
            ...
            params.extend([range_name, f'%{range_name}%', f'%{range_name}%', f'%{range_name}%'])
        ...
        query += " ORDER BY RANGE_LABEL, PRODUCT_IDENTIFIER LIMIT 5000"  # Increased limit
    ...
    def find_replacement_products(self, ranges: List[str], context: DocumentContext) -> List[Dict[str, Any]]:
        """Find commercialized replacement products for obsolete ranges"""
        if not ranges:
            return []
        
        where_conditions = ["COMMERCIAL_STATUS = '08-Commercialised'"]
        params = []
        ...
        # Look for similar ranges or product families
        range_conditions = []
        for range_name in ranges:
            # Find products in same category but commercialized
            range_conditions.extend([
                "UPPER(RANGE_LABEL) LIKE UPPER(?)",
                "UPPER(PRODUCT_DESCRIPTION) LIKE UPPER(?)",
                "UPPER(DEVICETYPE_LABEL) LIKE UPPER(?)"
            ])
            params.extend([f'%{range_name[:3]}%', f'%{range_name}%', f'%{range_name}%'])
        
        base_query = "SELECT * FROM products"
        if where_conditions and range_conditions:
            all_conditions = " WHERE " + " AND ".join(where_conditions)
            range_filter = " AND (" + " OR ".join(range_conditions) + ")"
            query = base_query + all_conditions + range_filter
        else:
            return []
        
        query += " ORDER BY RANGE_LABEL, PRODUCT_IDENTIFIER LIMIT 1000"
...
class IndustrialHTMLGenerator:
    """Industrial-grade HTML report generator with enhanced features"""
    
    def generate_report(self, results: List[ProcessingResult]) -> str:
        """Generate enhanced industrial-grade HTML report"""
...
<head>
    ...
    <title>SE Letters Industrial Pipeline V2 - {timestamp}</title>
    <style>
        * {{
            margin: 0;
            padding: 0;
        ...
        body {{
            ...
            color: #e0e0e0;
            font-size: 11px;  /* Much smaller base font */
            line-height: 1.3;
            min-height: 100vh;
            overflow-x: hidden;
        }}
        ...
        .header .subtitle {{
            font-size: 9px;  /* Much smaller subtitle */
            color: #bbb;
        ...
        .metric-value {{
            font-size: 9px;  /* Smaller metrics */
            font-weight: bold;
            color: #ffd23f;
        }}
        
        .metric-label {{
            font-size: 7px;  /* Tiny labels */
            color: #999;
            text-transform: uppercase;
            letter-spacing: 0.3px;
        }}
        ...
        .ai-method {{
            font-size: 7px;  /* Tiny method */
            color: #bbb;
        }}
        
        .ai-confidence {{
            font-size: 7px;  /* Tiny confidence */
            color: #27ae60;
            font-weight: bold;
        }}
        ...
        .overview-value {{
            font-size: 16px;  /* Slightly larger for overview */
            font-weight: bold;
            color: #ffd23f;
            margin-bottom: 2px;
        }}
        ...
        .tab {{
            ...
            cursor: pointer;
            font-family: inherit;
            font-size: 9px;  /* Smaller tab text */
            font-weight: bold;
            text-transform: uppercase;
            letter-spacing: 0.5px;
            transition: all 0.2s ease;
        ...
        .products-table {{
            ...
            font-size: 8px;  /* Much smaller table text */
        }}
        ...
        .metadata-content {{
            font-size: 8px;  /* Tiny metadata text */
            color: #ddd;
            line-height: 1.3;
        }}
        ...
        .grok-raw {{
            ...
            font-size: 7px;  /* Very small for raw data */
            color: #0f0;
            max-height: 200px;
            overflow-y: auto;
...
<body>
    <div class="industrial-container">
        <div class="sidebar scrollbar">
            <div class="header">
                <h1>📄 Documents</h1>
                <div class="subtitle">Industrial Analysis Pipeline V2</div>
            </div>"""
            ...
            <div class="document-thumbnail {active_class}" onclick="showDocument({i})">
                <div class="thumb-header">
                    ...
                    <div class="thumb-info">
                        <div class="thumb-title">{result.file_name[:30]}{'...' if len(result.file_name) > 30 else ''}</div>
                        <span class="{status_class}">{status_icon} {'OK' if result.success else 'ERR'}</span>
                    </div>
                </div>
                ...
                <div class="thumb-metrics">
                    ...
                    <div class="metric-item">
                        <div class="metric-value">{result.replacement_count}</div>
                ...
                <div class="ai-metadata">
                    ...
                    <div class="ai-confidence">Conf: {ai_confidence}</div>
                </div>
            </div>"""
        ...
        <div class="main-panel scrollbar">
            <div class="header">
                <h1>🏭 SE Letters Industrial Pipeline V2</h1>
                <div class="subtitle">Enhanced with Show All Products | Grok Raw Metadata | Replacement Suggestions</div>
            ...
            <div class="main-content">
                <div class="overview-panel">
                    ...
                    <div class="overview-grid">
                        ...
                        <div class="overview-card">
                            <div class="overview-value">{total_replacements:,}</div>
                            <div class="overview-label">Replacements</div>
                        </div>
                        <div class="overview-card">
                        ...
                        <div class="overview-card">
                            <div class="overview-value">{avg_reduction:.1f}%</div>
                            <div class="overview-label">Avg Reduction</div>
                        </div>
                        <div class="overview-card">
                ...
                <div class="tabs">
                    ...
                    <button class="tab" onclick="showTab(event, 'replacements')">🔄 Replacements</button>
                ...
                <div id="replacements" class="tab-content">
                    <div id="replacements-display">
                        <div class="no-products">
                            <h3>Select a document to view replacement suggestions</h3>
                            <p>Commercialized products that can replace obsolete items</p>
                        </div>
                    </div>
                </div>
                ...
                <div id="metadata" class="tab-content">
                    <div id="metadata-display">
                        <div class="no-products">
                            ...
                            <p>Complete extraction strategy breakdown with Grok raw data</p>
                        </div>
                    </div>
                </div>
                ...
                <div id="analytics" class="tab-content">
                    <div id="analytics-display">
                        <div class="no-products">
                            ...
                            <p>Processing performance and Grok raw metadata analysis</p>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        let showingAllProducts = {{}};  // Track which documents are showing all products
        ...
        function showDocument(index) {{
            ...
            if (activeTab.includes('products')) {{
                showProducts(index);
            }} else if (activeTab.includes('replacements')) {{
                showReplacements(index);
        ...
        function showProducts(docIndex) {{
            ...
            if (!result.products || result.products.length === 0) {{
                container.innerHTML = `
                    <div class="warning-box">
                        ...
                        <p><strong>Ranges:</strong> ${{result.ranges ? result.ranges.join(', ') : 'None'}}</p>
            ...
            const showingAll = showingAllProducts[docIndex] || false;
            const productsToShow = showingAll ? result.products : result.products.slice(0, 50);
                ...
                <div class="products-container">
                    <div class="products-header">
                        ...
                        <div class="products-info">
                            ...
                            <strong>Total:</strong> ${{result.products.length.toLocaleString()}} products
                        </div>
                    </div>
            ...
            if (!showingAll && result.products.length > 50) {{
                html += `
                    <button class="show-more-btn" onclick="showAllProducts(${{docIndex}})">
                        Show All ${{result.products.length.toLocaleString()}} Products
                    </button>
                `;
            }} else if (showingAll && result.products.length > 50) {{
                html += `
                    <button class="show-more-btn" onclick="showLessProducts(${{docIndex}})">
                        Show Less (First 50 Products)
                    </button>
                `;
            }}
        ...
        function showAllProducts(docIndex) {{
            showingAllProducts[docIndex] = true;
            showProducts(docIndex);
        }}
        
        function showLessProducts(docIndex) {{
            showingAllProducts[docIndex] = false;
            showProducts(docIndex);
        }}
        
        function showReplacements(docIndex) {{
            const result = documentsData[docIndex];
            const container = document.getElementById('replacements-display');
            
            if (!result.success) {{
                container.innerHTML = `
                    <div class="error-box">
                        <h3>❌ No Replacement Analysis Available</h3>
            ...
            if (!result.replacement_products || result.replacement_products.length === 0) {{
                container.innerHTML = `
                    <div class="warning-box">
                        <h3>⚠️ No Replacement Products Found</h3>
                        <p><strong>Ranges:</strong> ${{result.ranges ? result.ranges.join(', ') : 'None'}}</p>
                        <p>No commercialized products found that could replace obsolete items in these ranges.</p>
                    </div>
                `;
                return;
            }}
            ...
            let html = `
                <div class="products-container">
                    <div class="products-header">
                        <div class="products-title">🔄 Replacement Suggestions for ${{result.file_name}}</div>
                        <div class="products-info">
                            <strong>Commercialized Products:</strong> ${{result.replacement_products.length.toLocaleString()}} | 
                            <strong>Status:</strong> 08-Commercialised | 
                            <strong>Context:</strong> ${{result.context.pl_services_hint}}
                        </div>
                    </div>
                    
                    <div style="padding: 8px;">
                        <input type="text" class="filter-input" placeholder="🔍 Filter replacement products..." onkeyup="filterTable(this, 'replacements-table-${{docIndex}}')">
                        
                        <div style="max-height: 400px; overflow-y: auto;" class="scrollbar">
                            <table class="products-table" id="replacements-table-${{docIndex}}">
                                <thead>
                                    <tr>
                                        <th>Product Code</th>
                                        <th>Description</th>
            ...
            result.replacement_products.slice(0, 100).forEach(product => {{
                const description = product.PRODUCT_DESCRIPTION || '';
                const truncatedDesc = description.length > 60 ? description.substring(0, 60) + '...' : description;
                ...
                html += `
                    <tr>
                        ...
                        <td>${{truncatedDesc}}</td>
                        <td>${{product.RANGE_LABEL || ''}}</td>
                        <td style="color: #27ae60; font-weight: bold;">${{product.COMMERCIAL_STATUS || ''}}</td>
                        <td>${{product.BU_LABEL || ''}}</td>
                        <td>${{product.DEVICETYPE_LABEL || ''}}</td>
                    </tr>
                `;
            }});
            
            if (result.replacement_products.length > 100) {{
                html += `
                    <tr>
                        <td colspan="6" style="text-align: center; font-style: italic; background: #1a1a1a; color: #ffd23f;">
                            <strong>... and ${{(result.replacement_products.length - 100).toLocaleString()}} more replacement products available</strong>
                        </td>
                    </tr>
                `;
            }}
        ...
        function showMetadata(docIndex) {{
            ...
            html += `
                        ...
                        <div class="metadata-section">
                            <h4>📝 Raw Response</h4>
                            <div class="metadata-content" style="font-family: monospace; font-size: 7px; max-height: 100px; overflow-y: auto;">
                                ${{aiMetadata.raw_response || 'No raw response available'}}
                            </div>
                        </div>
                    </div>
                    
                    <div class="metadata-section" style="margin-top: 8px;">
                        <h4>🤖 Grok Raw Metadata</h4>
                        <div class="grok-raw">
${{aiMetadata.grok_raw_data || 'No Grok raw data available'}}
                        </div>
                    </div>
                </div>
            `;
        ...
        function showAnalytics(docIndex) {{
            ...
            let html = `
                <div class="metadata-panel">
                    <div class="metadata-title">📊 Document Analytics & Grok Raw Data</div>
                    <div class="metadata-grid">
                        <div class="metadata-section">
                        ...
                        <div class="metadata-section">
                            ...
                            <div class="metadata-content">
                                ...
                                <strong>Ranges Found:</strong> ${{result.ranges ? result.ranges.length : 0}}<br>
                                <strong>Products Matched:</strong> ${{result.product_count.toLocaleString()}}<br>
                                <strong>Replacements Found:</strong> ${{result.replacement_count.toLocaleString()}}<br>
                        ...
                        <div class="metadata-section">
                            ...
                            <div class="metadata-content">
                                ...
                                <strong>Efficiency Score:</strong> ${{(result.extraction_confidence * result.search_space_reduction / 100).toFixed(2)}}<br>
                                <strong>Replacement Ratio:</strong> ${{result.product_count > 0 ? (result.replacement_count / result.product_count * 100).toFixed(1) : 0}}%
                            </div>
                        </div>
                        ...
                        <div class="metadata-section">
                            ...
                            <div class="metadata-content">
                                <strong>Business Context:</strong> ${{result.context.business_context && result.context.business_context.length > 0 ? result.context.business_context.join(', ') : 'None detected'}}<br>
                                <strong>Document Type:</strong> ${{result.file_path.split('.').pop().toUpperCase()}}<br>
                                <strong>Processing Status:</strong> ${{result.success ? 'Successful' : 'Failed'}}<br>
                                <strong>Obsolescence Detected:</strong> ${{aiMetadata.validation_flags && aiMetadata.validation_flags.obsolescence_detected ? 'YES' : 'NO'}}
                            </div>
                        </div>
                        ...
                        <div class="metadata-section">
                            ...
                            <div class="metadata-content">
                                ...
                                <strong>Business Value:</strong> ${{result.product_count > 1000 ? 'HIGH' : result.product_count > 100 ? 'MEDIUM' : result.product_count > 0 ? 'LOW' : 'NONE'}}<br>
                                <strong>Replacement Potential:</strong> ${{result.replacement_count > 100 ? 'HIGH' : result.replacement_count > 10 ? 'MEDIUM' : result.replacement_count > 0 ? 'LOW' : 'NONE'}}
                            </div>
                        </div>
                    </div>
                    
                    <div class="metadata-section" style="margin-top: 8px;">
                        <h4>🤖 Complete Grok Raw Metadata</h4>
                        <div class="grok-raw">
${{aiMetadata.grok_raw_data || 'No Grok raw data available'}}
                        </div>
                    </div>
...
class SELettersIndustrialPipelineV2:
    """Enhanced Industrial-grade SE Letters Pipeline V2"""
    ...
    def run_pipeline(self, num_docs: int = 5) -> str:
        """Run the enhanced industrial pipeline"""
        print("🏭 SE LETTERS INDUSTRIAL PIPELINE V2 - ENHANCED EDITION")
        print("=" * 100)
        print("🤖 Show All Products | 📊 Grok Raw Metadata | 🔄 Replacement Suggestions | 📋 Overview Dashboard")
        print()
        ...
        print(f"📄 Processing {len(selected_docs)} documents with enhanced analysis")
        ...
        for i, doc_file in enumerate(selected_docs, 1):
            ...
            # 3. Enhanced range extraction with Grok simulation
            ...
            print(f"  🤖 Grok: {'Simulated' if ai_metadata.grok_raw_data else 'No data'}")
            ...
            # 5. Find replacement products (commercialized alternatives)
        ...
        print(f"\n🏭 Generating enhanced industrial-grade HTML report...")
        html_content = self.html_generator.generate_report(results)
        
        # Save report
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        report_path = self.output_dir / f"SE_Letters_Industrial_V2_Report_{timestamp}.html"
        ...
        print(f"\n🏆 ENHANCED INDUSTRIAL PIPELINE COMPLETE")
...
if __name__ == "__main__":
    pipeline = SELettersIndustrialPipelineV2()
    try:
        report_path = pipeline.run_pipeline(5)
        if report_path:
            print(f"\n🌐 Opening enhanced industrial report in browser...")
            import subprocess
